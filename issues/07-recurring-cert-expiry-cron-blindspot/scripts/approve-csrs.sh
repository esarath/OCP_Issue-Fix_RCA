...
# ─── Preflight: verify API is reachable before trusting any check below ──────
# Without this, "oc get csr" failing (e.g. API unreachable during a full
# cluster shutdown) looks identical to "genuinely zero pending CSRs" because
# errors are redirected to /dev/null below — the log would falsely say
# "healthy" for as long as the API stays down.
